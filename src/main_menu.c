...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, PALETTES_BG, SPECIES_MISSINGNO);
...
{
    return CreateMonPicSprite_Affine(SPECIES_MISSINGNO, SHINY_ODDS, 0, MON_PIC_AFFINE_FRONT, x, y, 14, TAG_NONE);
}
...
static void RenderPlayerParty(void)
{
    ...
    for (i = 0; i < gPlayerPartyCount; i++)
    {
        ...
        id = CreateMonIconNoPersonality(species, SpriteCallbackDummy, 32 * i + 40, 64, 0, FALSE);
        iconsIDs[i] = id;
